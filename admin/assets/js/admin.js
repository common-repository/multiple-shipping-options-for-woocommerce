...
jQuery(document).ready(function () {
    ...
    // Iterate over input elements with value starting with "mso_shipment_order_"  
    jQuery('table#list-table tbody input[value^="mso_shipment_order_"]').each(function () {
        // Get the input ID
        var inputId = jQuery(this).attr('id');

        // Check if the input has an id attribute and if it contains "-key"
        if (inputId && inputId.includes('-key')) {
            // Remove "-key" suffix from the input ID
            var trInputId = inputId.replace('-key', '');

            // Hide the corresponding table row
            jQuery('tr#' + trInputId).hide();
        }
    });
    ...
    // ============================================ NEW ============================================
    // Make disabled fields read-only and non-clickable
    jQuery('textarea.mso_disabled, input[type="number"].mso_disabled, input[type="text"].mso_disabled').attr('readonly', 'readonly');
    jQuery('select.mso_disabled').prop('disabled', true);
    jQuery('input[type="checkbox"].mso_disabled').prop('checked', false);

    // Show tooltip for disabled interactive elements
    jQuery('input[type="checkbox"].mso_disabled, input[type="radio"].mso_disabled, textarea.mso_disabled, input[type="number"].mso_disabled, input[type="text"].mso_disabled, select.mso_disabled').on('click', function (ev) {
        ev.preventDefault();

        jQuery('.mso_c_wrapper').remove();

        const tooltip_text = mso_script.mso_paid_plan_feature;
        const tooltip = `<span class="mso_c_wrapper"><span class="mso_tooltip" style="opacity: 1;">${tooltip_text}</span></span>`;

        jQuery(this).after(tooltip);
    });

    // Remove tooltip when clicking outside disabled elements
    jQuery(document).on('click', function (e) {
        const $closest_disabled = jQuery(e.target).closest('.mso_disabled');

        if ($closest_disabled.length === 0) {
            jQuery('.mso_c_wrapper').remove();
        }
    });
    ...
    jQuery('.mso_get_updated_plan_status').on('click', function (event) {
        event.preventDefault();
        jQuery('.mso_license_api_status label.mso_api_credentials_status_btn').click();
        jQuery(document).on('mso_roller_overly_removed', function () {
            // console.log('mso_roller_overly removed!');
    ...
    jQuery("input[name='mso_residential'],input[name='mso_liftgate'],span.mso_item_quantity input[type='number'],select.mso_order_shipment_origin, input[id='mso_ship_to_address']").on('change', function () {
    ...
    jQuery('.mso_pp_solution').on('click', function (event) {
        ...
        let params = {
            ...
            'mso_cid': mso_cid,
            'loader_id': '.mso_pallets_boxes_page'
        };
    ...
    // Define a function to set data attributes for a service type and carrier
    function set_service_data_attributes(selector, service_type, carrier_id) {
    ...
    // Set data attributes for domestic services
    set_service_data_attributes('#mso_dhl_spq_express_services_sa', 'domestic', 'dhl');
    set_service_data_attributes('#mso_usps_spq_domestic_services_sa', 'domestic', 'usps');
    set_service_data_attributes('#mso_fedex_spq_domestic_services_sa', 'domestic', 'fedex');
    set_service_data_attributes('#mso_ups_spq_domestic_services_sa', 'domestic', 'ups');

    // Set data attributes for international services
    set_service_data_attributes('#mso_usps_spq_international_services_sa', 'international', 'usps');
    set_service_data_attributes('#mso_fedex_spq_international_services_sa', 'international', 'fedex');
    set_service_data_attributes('#mso_ups_spq_international_services_sa', 'international', 'ups');


    // Handle click event for USPS services
    jQuery('.mso_dhl_dsa, .mso_usps_dsa, .mso_usps_isa,.mso_ups_dsa, .mso_ups_isa,.mso_fedex_dsa, .mso_fedex_isa').on('click', function () {
        const $selectedService = jQuery(this);
        const $selectedAll = $selectedService.closest('tr')
            .prevUntil('tr.mso_services_sa')
            .prev('tr.mso_services_sa')
            .find('td label input');

        if (!$selectedAll.hasClass('mso_disabled')) {
            const creating_class = mso_prev_relative_class($selectedAll);

            if (jQuery(creating_class).length) {
                const isChecked = jQuery('input' + creating_class + ':checked').length === jQuery('input' + creating_class).length;
                const main_id = $selectedAll.attr('id');
                jQuery('#' + main_id).prop("checked", isChecked);
            }
        }
    });

    // Handle click event for service selection
    jQuery('.mso_services_sa').on('click', function () {
        const $selected = jQuery(this);

        if (!$selected.hasClass('mso_disabled')) {
            const creating_class = mso_prev_relative_class($selected);

            const $target_inputs = jQuery('input' + creating_class);

            if ($target_inputs.length) {
                const is_checked = $selected.prop('checked');

                $target_inputs.prop('checked', is_checked);
            }
        }
    });
    ...
    jQuery('.mso_hidden_table_th').closest('tr').addClass("mso_hidden_table_th");
    ...
    jQuery('.mso_save_packaging').on('click', function (event) {
        ...
        jQuery('div.' + mso_maindiv + ' .mso_packaging_td_row').each(function (ind, template) {
            ...
            mso_packaging_data[mso_packaging_count] = jQuery(template).find('input,select').serialize();
            mso_packaging_count++;
        });
    ...
    jQuery('.mso_add_packaging').on('click', function (event) {
        ...
        jQuery(mso_linked_div).last().find('select').val('default');
...
// Define a function to find the previous relative class
if (typeof mso_prev_relative_class != 'function') {
    function mso_prev_relative_class($selectedAll) {
        const service = $selectedAll.attr('data-service');
        const service_id = $selectedAll.attr('data-id');
        let creating_class = '.mso';

        if (service_id.length > 0) {
            creating_class += '_' + service_id;
        }
...
// Ship confirmation
if (typeof mso_order_proceed_ship_overly_hide != 'function') {
    function mso_order_proceed_ship_overly_hide() {
        jQuery('.mso_order_shipment_proceed_overly').css({'opacity': 0, 'display': 'none'});
        jQuery('.mso_order_shipment_proceed_overly').hide();
    }
}
...
if (typeof mso_order_shipment_remove != 'function') {
    function mso_order_shipment_remove(obj, event) {
        ...
        jQuery('.mso_delete_shipment_done').on('click', function () {
            ...
            jQuery(remove_template).prev('.mso_order_ship_action').remove();
...
if (typeof mso_order_ship != 'function') {
    function mso_order_ship() {
        ...
        var params = {
            ...
            'mso_ship_to_address': mso_ship_to_address,
            'mso_order_id': mso_order_id,
            'ship_action': 'confirmation',
            'loader_id': '#mso_order',
        };
        mso_ajax_request(params, mso_shipment_before_order_ship_proceed);

        jQuery('.mso_order_shipment_proceed_overly').css({'opacity': 1, 'display': 'block'});
        jQuery('.mso_proceed_shipment_done').on('click', function (e) {

            if (mso_validate_input('.ship_confirmation_child') == false) {
                return false;
            }

            jQuery('.ship_confirmation_child').each(function () {
                var ship_package = jQuery(this).data('ship_package');
                var shipment_settings = {};
                jQuery(this).find("input[type='text']").each(function () {
                    var text_field_name = jQuery(this).attr('name');
                    shipment_settings[text_field_name] = jQuery(this).val();
                });

                mso_shipment[ship_package]['shipment_settings'] = shipment_settings;
            });
...
// Order confirmation
if (typeof mso_shipment_before_order_ship_proceed != 'function') {
    function mso_shipment_before_order_ship_proceed(params, response) {
        jQuery('.ship_confirmation_dialog').remove();
        jQuery('.mso_proceed_message').after(response);
    }
}
...
if (typeof mso_shipment_order != 'function') {
    function mso_shipment_order(params, response) {
        ...
        // API Response empty
        jQuery('div.mso_order_api_response').html('');
        var parsed_response = JSON.parse(response);
        var shipment_rates = parsed_response.order_rates;
        // Packaging
        var order_packages = parsed_response.order_packages;
        // API Response
        var api_response = parsed_response.api_response;
        var rates_returning = parsed_response.rates_returning;
        var error_message = parsed_response.error_message;
        if (mso_is_empty(shipment_rates)) {
            mso_display_li_option('msoorc');
            const error_message_display = mso_is_empty(error_message) ? 'Please try again later.' : error_message;
            jQuery('div.mso_order_rates').last().append(`<span style="color: red;"><b>Error! </b>${error_message_display}</span>`);
        } else {
            jQuery(".mso_order_shipment").each(function (index, obj) {
                var mso_shipment_number = jQuery(obj).data('mso_ship_num');
                    ...
                    if (mso_shipment_number == mso_ship_num) {
                        if (rate.length > 0) {
                            ...
                            if (typeof order_packages[mso_ship_num] != 'undefined') {
                                ...
                                // API Response
                                var mso_order_api_response = '<div class="mso_order_api_response mso_order_tab">' +
                                    '<p class="mso_calculate_shipping_heading">API Response</p>' +
                                    api_response[mso_ship_num] +
                                    '</div>';

                                jQuery(obj).next().find('div.mso_order_api_response').html(mso_order_api_response);
...
if (typeof mso_delete_packaging != 'function') {
    function mso_delete_packaging(obj, event, package) {
        if (!jQuery(obj).hasClass('mso_disabled')) {
            ...
            var delete_message = 'Are you sure you want to delete the ' + package + '?';
            jQuery('.mso_package_delete_action .mso_delete_message p').text(delete_message);
    ...
    function mso_validate_input(template) {
        ...
        jQuery(template).find("input[type='text']").each(function () {
            ...
            if (error_text.length === 0) {
                error_text = jQuery(this).attr('placeholder');
            }
...
// Update plan status
function mso_get_updated_plan_status(params) {
    mso_ajax_request(params, mso_get_updated_plan_status_proceed);
}

// Update plan status proceed
function mso_get_updated_plan_status_proceed(params, response) {
    var pr = JSON.parse(response);
    if (pr.action && pr.message && pr.action === true) {
        alert('Your updated plans have been successfully applied and are now listed within the plugin.');
    } else {
        alert('Your current plan subscription is inactive.');
    }
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                if (jQuery('.mso_roller_overly').length) {
                    jQuery('.mso_roller_overly').remove();
                    jQuery(document).trigger('mso_roller_overly_removed');
                }
